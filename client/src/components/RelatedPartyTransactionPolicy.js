...
- *Consequences of Non-Compliance*: Outline consequences of non-compliance with the policy, including disciplinary action and reporting to regulatory authorities.`;
    ...
    try {
        ...
        onResponse: async (data) => {
          ...
          if (data && data.candidates && data.candidates[0] && data.candidates[0].content) {
            ...
            await trackActivity({
              ...
              feature: FEATURES.RELATED_PARTY_TRANSACTION_POLICY,
              action: 'Related Party Transaction Policy Generated Successfully',
              inputData: {
            ...
            await trackActivity({
              activityType: ACTIVITY_TYPES.POLICY_GENERATION,
              feature: FEATURES.RELATED_PARTY_TRANSACTION_POLICY,
              action: 'Related Party Transaction Policy Generation Failed',
              inputData: {
                companyName: formData.companyName
              },
              outputData: {
      ...
      await trackActivity({
        activityType: ACTIVITY_TYPES.POLICY_GENERATION,
        feature: FEATURES.RELATED_PARTY_TRANSACTION_POLICY,
        action: 'Related Party Transaction Policy Generation Error',
        inputData: {
          companyName: formData.companyName
        },
        outputData: {
