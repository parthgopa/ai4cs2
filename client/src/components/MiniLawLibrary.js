...
- If no reported case law is found, clearly state: "No reported judgments available for this section."`;
    ...
    try {
        ...
        onResponse: async (data) => {
          ...
          if (data && data.candidates && data.candidates[0] && data.candidates[0].content && data.candidates[0].content.parts) {
            ...
            await trackActivity({
              ...
              feature: FEATURES.MINI_LAW_LIBRARY,
              action: 'Law library search completed successfully',
              inputData: {
            ...
            await trackActivity({
              activityType: ACTIVITY_TYPES.DOCUMENT_GENERATION,
              feature: FEATURES.MINI_LAW_LIBRARY,
              action: 'Failed to complete law library search',
              inputData: {
                searchQuery: formData.searchQuery
              },
              outputData: {
      ...
      await trackActivity({
        activityType: ACTIVITY_TYPES.DOCUMENT_GENERATION,
        feature: FEATURES.MINI_LAW_LIBRARY,
        action: 'Error in law library search',
        inputData: {
          searchQuery: formData.searchQuery
        },
        outputData: {
